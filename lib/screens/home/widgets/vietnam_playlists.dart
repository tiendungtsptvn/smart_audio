...
class VietnamPlaylists extends GetView<HomeController> {
  const VietnamPlaylists({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      List<PlaylistSimple> playlists = [];
      bool loading = controller.vietnamPlaylists.isEmpty && controller.loadingVietnamPlaylists;
      if(loading){
        playlists = List.generate(3, (index) => PlaylistSimple());
      }else{
        playlists = controller.vietnamPlaylists;
      }
